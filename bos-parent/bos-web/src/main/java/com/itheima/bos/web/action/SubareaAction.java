 ...
 * @throws IOException 
   ...
   /**
    * 查询所有未关联到定区的分区的方法 返回json数据
    */

	public String listajax() throws Exception {
		List<Subarea> list =  subareaService.findListNotAssociation();
		this.java2json(list, new String[]{"decidedzoom","region"}); //把关联对象排除
		return NONE;
	}
	/**
	 * 根据定区的id 查询关联的分区
	 */
	private String decidedzoneId; //属性驱动,接收定区的id
	public  String findListByDecidedzoneId(){
	  List<Subarea>  list=	subareaService.findListByDecidedzoneId(getDecidedzoneId()); //定区的id,没有
	  //页面的分区 有可能会属于一个区域,在转json的时候,关联的region不能排除   没有定区的字段,可以把定区首先排除 然后把region变小成立即加载
	  this.java2json(list, new String[]{"subareas","decidedzoom"});
		return NONE;
	}
